...
dat_prepped <-  dat_merged %>% 
  ...
  mutate(top_trait = str_split_i(question, pattern = "/", i = 2), 
         bottom_trait = str_split_i(question, pattern = "/", i = 1)) %>% 
  mutate(avg_rating = case_when(personality == top_trait ~ avg_rating, 
...
p_robin <- ggplot(data = dat_prepped_robin, 
...
p_rest <- ggplot(data = dat_prepped %>% filter(char_name != "Robin Scherbatsky"), 
        mapping = aes(x = question, y = avg_rating, shape = uni_name_fac)) +
  geom_point() +
...
dat_prepped3 <- dat_prepped3 %>% 
  mutate(image_x = ifelse(uni_name == "Friends", -1, 1), 
...
ggplot(dat_prepped3, 
  ...
  geom_image(aes(x = image_x_coord, y = 100, image = image_link), 
             nudge_x = c(0.5, -0.5), 
             size=0.1, inherit.aes = FALSE,   asp = 1,
  mask = "circle") +
  NULL
```
