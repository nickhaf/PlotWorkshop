...
Ladet die `characters`-Daten [hier](https://github.com/nickhaf/PlotWorkshop/blob/main/data/characters.rds) herunter, und legt sie ebenfalls in eurem `data`-Ordner ab. 
...
```{r}
#| output-location: column
#| code-line-numbers: "1,2,3,4,5,6,7,13,16,17,18,19"

library(ggtext)

char_prepped_2 <- char_prepped %>%
  mutate(char_name_label = case_when(
    char_name == "Monica Geller" & top_trait == "orderly" ~ "Monica ist sehr <span style='color:green'>ordentlich</span>",
    TRUE ~ NA
  ))

ggplot(
  data = char_prepped_2,
  mapping = aes(x = top_trait, 
                y = avg_rating, 
                label = char_name_label)
) +
  geom_point() +
  geom_richtext(
...
# Themes
Zum Einstellen von [nicht datenbezogene]{.highlight} Elementen eines Plots (z.B. `plot.title` oder `axis.ticks.x`)
...
## Themes: Anpassen
Funktioniert über `theme()`. 
Lass dir mit `?theme` anzeigen, was man alles verändern kann. 
Je nach Art des Elements (z.B. Text, oder Fläche (z.B. bei Plot-Hintergrund)) muss ein anderes `element_` genutzt werden: 
