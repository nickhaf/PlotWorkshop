...
::: {.columns}
:::{.column width="50%"}
![](./images/garbage_hochkant.jpg){.image-left}
...
- [Sloppy reserach](https://journals.sagepub.com/doi/full/10.1177/0306312714535679)
  - P-Hacking
  - Cherry Picking
...
## Fehlender Kontext  {.center}

![](./images/impeach.png)
::: images-ref
Sie [storymaps](https://storymaps.arcgis.com/stories/0e636a652d44484b9457f953994b212b).
:::
...
“The designer of the visualization, by selecting a y-axis starting point, has control over the subjective importance of the resulting differences.” [@correll2020truncating]
...
"If zero is in the neighborhood, invite it in!"[Andrew Gelman](https://statmodeling.stat.columbia.edu/2021/12/17/graphing-advice-if-zero-is-in-the-neighborhood-invite-it-in/)
...
# Story
Rolfs 7
Nature https://www.nature.com/articles/nmeth.2571


## Erkläre Auffälligkeiten
...
gapminder_line <- gapminder_dat_full %>%
  filter(country %in% c("deu"), time >= 1850, time <= 2024) %>%
  drop_na(gini)


steuer <- gapminder_line %>% 
filter(time %in% c(1883, 1923, 1952, 1974, 1990, 1997, 2020)) %>% 
arrange(time) %>% 
mutate(label = c("Einführung des Preußischen Ergänzungsgesetz",  "Einführung der Vermögenssteuer nach preußischem Vorbild", "Vermögenssteuergestz", "Heutiges Gesetz", "Wiedervereinigung", "Aussetzung der Vermögenssteuer", "Corona"))
...
p_line <- ggplot(data = gapminder_line, aes(x = time, y = gini)) +
 ...
 annotate(
    "label",
    x = 1850, y = 30,        # Ecke unten links
    hjust = 0, vjust = 0,      # innen ausrichten
    label = "\bold(Gini-Koeffizient): Maß für Ungleichheit.\n0 = vollkommen gleich, 100 = maximal ungleich.",
    size = 4,                  # Textgröße
    label.size = 0.5,          # Rahmenstärke (0 = ohne)
    label.r = grid::unit(3, "pt"),
    fill = "white",            # oder scales::alpha("white", 0.9)
    colour = "black")

p_line
```

Zweiten Plot hierdraus!
