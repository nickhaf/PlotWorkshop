...
::: {.absolute right="5%" top="5%" style="font-size:2em; color:#F8F8F8; padding: 0.5em 0.5em; background-color: rgba(255, 255, 255 .7); backdrop-filter: blur(5px); box-shadow: 0 0 1rem 0 rgba(255, 255, 255 .5); border-radius: 5px;"}
...
```{r}
# Auswahl der Spalten
gapminder_dat_full[c(1,4), c("country", "time")]
```

## Konditionales Subsettting
```{r, output.lines = 2}
gapminder_dat <- gapminder_dat_full[gapminder_dat_full$name == "Germany", ]
head(gapminder_dat)
```
...
```{r, output.lines = 2}
gapminder_dat <- gapminder_dat_full %>% 
  filter(gapminder_dat_full$name == "Germany")
head(gapminder_dat)
```


## Komplexes konditionales Subsetting: [tidyverse](https://www.tidyverse.org/)
...
## Faktoren
Eventuell auh einfach später einführen, wenn gebraucht!

```{r}
gapminder_dat_full$country
```


```{r}
as.factor(gapminder_dat_full$country)
```
